...
this.axiosInstance = axios.create({
     ...
     */
// im Klassen-Body von Twinkly
async getPlaylist() {
    this.playlist = {};

    let response;
    try {
        response = await this._get('playlist');
    } catch (e) {
        // alle Parser-Fehler (Header defekt, 204+Body, Connection reset …)
        if (e.message && e.message.includes('Parse Error')) {
            return {
                unique_id : '',
                name      : '',
                entries   : [],
                code      : HTTPCodes.values.ok
            };
        }
        throw e;          // echte Fehler weiterreichen
    }
...
async getCurrentPlaylistEntry() {
    try {
        ...
        // Twinkly schickt 204 + kaputten Body → als „keine Playlist aktiv“ behandeln
        if (e.message && (e.message.includes('Parse Error') || e.message.includes('(204)'))) {
            return { id: -1, unique_id: '', name: '', duration: 0, code: HTTPCodes.values.ok };
        }
        throw e;      // echte Fehler weitergeben
    }
}
